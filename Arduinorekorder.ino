...
void loop() {
  ...
  if(digitalRead(2) == 1){                                                          //Dersom bryteren er tatt opp og ned 
    ...
    if(ss1%2 == 0 && started){
      ...
      audio.startRecording("lydfilnavn.wav",32000,A0);                                  //("lydfilnavn.wav", samplefrekvensen, analog utgangspin)
      delay(1000);
      ...
      audio.stopRecording("lydfilnavn.wav");                                            //Opptaket blir lagret under ("lydfilnavn.wav)
      started = true; 
      stopped = false;
      delay(1000);
      }
